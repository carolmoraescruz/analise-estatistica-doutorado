...
setwd("C:/Users/Dell/Documents/Cursos/Academicos/Doutorado em Ciencia e Engenharia de Materiais/Tese/analise-estatistica-doutorado-repo/analise-r/etapa-2")
...
# Importar os dados
MEA_7dias <- read.csv("MEA_A_7dias.csv", header = TRUE)
RFTA_7dias <- read.csv("RFTA_A_7dias.csv", header = TRUE)
RCTA_7dias <- read.csv("RCTA_A_7dias.csv", header = TRUE)


# Mudar o tipo dos dados de "wide" para "long"
MEA7_stack <- stack(MEA_7dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(MEA7_stack) <- c("MEA", "Formula")

RFTA7_stack <- stack(RFTA_7dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(RFTA7_stack) <- c("RFTA", "Formula")

RCTA7_stack <- stack(RCTA_7dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
...
# Importar os dados
MEA_28dias <- read.csv("MEA_A_28dias.csv", header = TRUE)
RFTA_28dias <- read.csv("RFTA_A_28dias.csv", header = TRUE)
RCTA_28dias <- read.csv("RCTA_A_28dias.csv", header = TRUE)


# Mudar o tipo dos dados de "wide" para "long"
MEA28_stack <- stack(MEA_28dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(MEA28_stack) <- c("MEA", "Formula")

RFTA28_stack <- stack(RFTA_28dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(RFTA28_stack) <- c("RFTA", "Formula")

RCTA28_stack <- stack(RCTA_28dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
...
# Importar os dados
MEA_91dias <- read.csv("MEA_A_91dias.csv", header = TRUE)
RFTA_91dias <- read.csv("RFTA_A_91dias.csv", header = TRUE)
RCTA_91dias <- read.csv("RCTA_A_91dias.csv", header = TRUE)


# Mudar o tipo dos dados de "wide" para "long"
MEA91_stack <- stack(MEA_91dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(MEA91_stack) <- c("MEA", "Formula")

RFTA91_stack <- stack(RFTA_91dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
names(RFTA91_stack) <- c("RFTA", "Formula")

RCTA91_stack <- stack(RCTA_91dias, select = c("A_N", "AA_0.5C", "AA_1.0C"))
